									...
									<tr>
										...
										<td>
											...
											while($shh=mysqli_fetch_array($tr))
											{
												...
												if($ttme=mysqli_fetch_array($ttm)){
													$ttm1=mysqli_query($con,"select  * from tbl_screens where screen_id='".$ttme['screen_id']."'");
													if($ttme1=mysqli_fetch_array($ttm1)){
												
												?>
												
												<a href="check_login.php?show=<?php echo $shh['s_id'];?>&movie=<?php echo $shh['movie_id'];?>&theatre=<?php echo $shw['theatre_id'];?>"><button class="btn btn-default"><?php echo $ttme1['screen_name'].'-';//echo $x= date('H:i');
												 echo $y=date('h:i A',strtotime($ttme['start_time']));
												//if($x>$ttme['start_time']){
                                                //echo "current greater";
												//}else{
                                               // echo "v";
												//}
